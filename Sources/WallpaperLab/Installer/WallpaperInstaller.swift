// 来源: YangJiiii/3105 (GPLv3),搬运/改写自 ThreeOneOSFive/helpers/WallpaperInstaller.swift。
import Foundation
import Darwin

struct WallpaperInstalledDescriptor: Codable, Equatable, Identifiable, Sendable {
...
enum WallpaperInstallStatus: String, Codable, Sendable {
    case preparing
    case installed
    case rolledBack
    case restored
...
struct WallpaperInstallReceipt: Codable, Equatable, Identifiable, Sendable {
    let id: UUID
    let createdAt: Date
    ...
    static func restore(
        ...
        do {
            for installed in receipt.installedDescriptors {
                ...
                let extensionDirURL = containerURL
                ...
                guard WallpaperLayoutScanner.isContained(extensionDirURL, in: containerURL) else {
                    throw WallpaperLabError.restoreFailed
                }
                // descriptor 容器目录与扫描器同规则探测(descriptors 优先,回退 configurations),
                // 老收据在容器目录已不存在时回退 descriptors,维持历史行为。
                let resolvedContainerDir = WallpaperLayoutScanner.descriptorContainerURL(
                    in: extensionDirURL,
                    fileManager: fileManager
                ) ?? extensionDirURL.appendingPathComponent("descriptors", isDirectory: true)
                let descriptorsURL = resolvedContainerDir
